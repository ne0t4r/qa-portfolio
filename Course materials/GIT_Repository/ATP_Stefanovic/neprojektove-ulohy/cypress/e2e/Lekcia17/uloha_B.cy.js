...
describe('Zadanie_17.3.1_B práca s "Multi Step Form"', () => {
    ...
    before('passes', () => {
      ...
      cy.fixture("30.3.1_B.json").then(function(data) {
        this.data = data;
    });
});
    it ("Test", function() {
        ...
        cy.get("#msf-text-your-name").clear().should("have.value", "").type(this.data.name).should("have.value", this.data.name);
        cy.get("#msf-mail-your-email").clear().should("have.value", "").type(this.data.email).should("have.value", this.data.email);
        cy.get("#msf-date-select-date-of-order").clear().should("have.value", "").type(this.data.date).should("have.value", this.data.date);         
        ...
        cy.get(".fw-textarea").type(this.data.message).should("have.value", this.data.message);
        ...
        cy.get(".fw-step-summary").eq(0).contains(this.data.name).should("be.visible");
        
        cy.get(".fw-step-summary-field-title").eq(1).contains("Your Email").should("be.visible");
        cy.get(".fw-step-summary").eq(1).contains(this.data.email).should("be.visible");
        
        cy.get(".fw-step-summary-field-title").eq(2).contains("Select Date Of Order").should("be.visible");
        cy.get(".fw-step-summary").eq(2).contains(this.data.date).should("be.visible");
        ...
        cy.get(".fw-step-summary").eq(4).contains("Shoes, Pants, Jackets, Hats, Shorts, T-shirts").should("be.visible");
        ...
        cy.get(".fw-step-summary-field-title").eq(6).contains("Write us a message").should("be.visible");
        cy.get(".fw-step-summary").eq(6).contains(this.data.message).should("be.visible");
