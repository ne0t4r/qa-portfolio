...
describe('Zadanie_17.3.1_A práca s "Multi Step Form"', () => {
    ...
    it ("Test", () => {
        ...
        cy.get("#msf-date-select-date-of-order").clear().should("have.value", "").type(date).should("have.value", date);         
        ...
        cy.get(".fw-textarea").type(message).should("have.value", message);
        ...
        cy.get(".fw-step-summary").eq(0).contains(name).should("be.visible");
        
        cy.get(".fw-step-summary-field-title").eq(1).contains("Your Email").should("be.visible");
        cy.get(".fw-step-summary").eq(1).contains(email).should("be.visible");
        
        cy.get(".fw-step-summary-field-title").eq(2).contains("Select Date Of Order").should("be.visible");
        cy.get(".fw-step-summary").eq(2).contains(date).should("be.visible");
        ...
        cy.get(".fw-step-summary").eq(6).contains(message).should("be.visible");
